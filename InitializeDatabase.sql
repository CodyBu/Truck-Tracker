...
CREATE SEQUENCE seqC01 INCREMENT 1 BY START WITH 1
...
CREATE TABLE MAINTENANCE_ENTRY(
  ...
  CONSTRAINT RequesterFK FOREIGN KEY (Requester) REFERENCES USER(UserID) ON UPDATE CASCADE ON DELETE SET NULL,
  CONSTRAINT MechanicFK FOREIGN KEY (Mechanic) REFERENCES USER(UserID) ON UPDATE CASCADE ON DELETE SET NULL
);
CREATE SEQUENCE seqC02 INCREMENT 1 BY START WITH 1
...
CREATE TABLE NOTE(
  ...
  CONSTRAINT UserFK FOREIGN KEY (User)  REFERENCES USER(UserID) ON UPDATE CASCADE ON DELETE SET NULL
);
